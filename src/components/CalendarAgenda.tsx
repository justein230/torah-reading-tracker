...
export function AgendaView({ year, dayMap, SEFER_MAP }: Readonly<AgendaViewProps>) {
  const monthGroups = useMemo(() => {
    const datesInYear = Object.keys(dayMap).filter(d => d.startsWith(String(year)));
    const groups = groupBy(datesInYear, d => Number.parseInt(d.slice(5, 7), 10) - 1);
    for (const dates of groups.values()) dates.sort((a, b) => a.localeCompare(b));
    return groups;
  }, [year, dayMap]);

  const monthKeys = [...monthGroups.keys()].sort((a, b) => a - b);
