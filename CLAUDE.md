...
**Manual UI testing / browser automation:** The dev server (`npm run concurrent`) usually is already running at `http://localhost:8000/` — check `ps -ef | grep -i "concurrently\|nodemon\|vite"` before starting another one. Auth mode is password-based locally; the admin password is `TORAH_ADMIN_PASSWORD` in `.env` (repo root). To log in via the UI: click the gear `ActionIcon` (top right) to open the Settings drawer, fill the `input[type="password"]` field, click "Log in". Without logging in, `canWrite` is false and write-gated UI (e.g. row edit/delete actions) won't render. For Playwright screenshots, the bundled `chromium.launch()` default executable often doesn't match the cached revision — find a working binary first with `find ~/.cache/ms-playwright -iname "chrome" -type f` and pass it as `executablePath`. Playwright itself lives in this project's `node_modules`, so run scripts with `node` from inside `/home/justein/torah` (not from a scratch/tmp dir) so module resolution finds it.
