...
// to thread a session cookie through each call individually.
...
describe('GET /api/can-write', () => {
  it('returns canWrite: true when logged in', async () => {
    const res = await agent.get('/api/can-write');
...
describe('POST /api/readings', () => {
  it('returns 201 and reading_type=standard for first reading', async () => {
    const res = await agent.post('/api/readings')
      .send({ parsha: 'בראשית', aliyah: 1, date_read: '2024-01-01' });
  ...
  it('stores occasion and location', async () => {
    ...
    const aliyot = await agent.get('/api/aliyot');
...
describe('PUT /api/readings/:id', () => {
  it('updates occasion and location', async () => {
    const post = await agent.post('/api/readings')
      .send({ parsha: 'בראשית', aliyah: 1, date_read: '2024-01-01', occasion: 'Old', location: 'Home' });
    const id = post.body.id;

    const res = await agent.put(`/api/readings/${id}`)
  ...
  it('returns 404 for a non-existent reading', async () => {
    const res = await agent.put('/api/readings/99999')
      .send({ occasion: 'x' });
    expect(res.status).toBe(404);
  });
});
...
describe('DELETE /api/readings/:id', () => {
  it('returns 204 and removes the reading', async () => {
    ...
    const del = await agent.delete(`/api/readings/${id}`);
  ...
  it('returns 404 for a non-existent reading', async () => {
    const res = await agent.delete('/api/readings/99999');
    expect(res.status).toBe(404);
  });
});
...
describe('GET /api/stats/location', () => {
  it('returns an array of location stats', async () => {
    const res = await agent.get('/api/stats/location');
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
  });
  ...
  it('counts weekday and hosafot readings alongside standard and special readings', async () => {
    const weekdayAliyahId = (await agent.get('/api/weekday-aliyot')).body[0].id;

    await agent.post('/api/readings')
      .send({ parsha: 'בראשית', aliyah: 1, date_read: '2024-01-01', location: 'Shul' });
    await agent.post('/api/readings/weekday')
...
describe('GET /api/hebcal', () => {
  it('returns a schedule object populated from the baked cache', async () => {
    const res = await agent.get('/api/hebcal');
    expect(res.status).toBe(200);
...
describe('GET /api/occasions', () => {
  it('returns occasions with the expected shape', async () => {
    const res = await agent.get('/api/occasions');
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
...
describe('GET /api/occasion-aliyot', () => {
  it('returns occasion aliyot with the expected shape', async () => {
    const res = await agent.get('/api/occasion-aliyot');
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    expect(res.body.length).toBeGreaterThan(0);
    const first = res.body[0];
...
describe('GET /api/readings/special', () => {
  it('returns an empty array when nothing has been read', async () => {
    const res = await agent.get('/api/readings/special');
    expect(res.status).toBe(200);
    expect(res.body).toEqual([]);
  });

  it('reflects a posted special reading', async () => {
    const aliyot = await agent.get('/api/occasion-aliyot');
    const occasionAliyahId = aliyot.body[0].id;
    await agent.post('/api/readings/special')
...
describe('POST /api/readings/special', () => {
  it('returns 201 and an id on success', async () => {
    const aliyot = await agent.get('/api/occasion-aliyot');
    const occasionAliyahId = aliyot.body[0].id;
    const res = await agent.post('/api/readings/special')
  ...
  it('returns 400 when occasion_aliyah_id is missing', async () => {
    const res = await agent.post('/api/readings/special').send({ date_read: '2024-01-01' });
    expect(res.status).toBe(400);
  });
...
describe('DELETE /api/readings/special/:id', () => {
  it('returns 204 and removes the special reading', async () => {
    const aliyot = await agent.get('/api/occasion-aliyot');
    const post = await agent.post('/api/readings/special')
      .send({ occasion_aliyah_id: aliyot.body[0].id, date_read: '2024-01-01' });
    const del = await agent.delete(`/api/readings/special/${post.body.id}`);
    expect(del.status).toBe(204);
    const res = await agent.get('/api/readings/special');
  ...
  it('returns 404 for a non-existent special reading', async () => {
    const res = await agent.delete('/api/readings/special/999999');
    expect(res.status).toBe(404);
  });
});
...
describe('GET /api/weekday-aliyot', () => {
  it('returns weekday aliyot with the expected shape', async () => {
    const res = await agent.get('/api/weekday-aliyot');
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    expect(res.body.length).toBeGreaterThan(0);
    const first = res.body[0];
...
describe('POST /api/readings/weekday', () => {
  it('returns 201 and an id on success', async () => {
    const aliyot = await agent.get('/api/weekday-aliyot');
    const weekdayAliyahId = aliyot.body[0].id;
    const res = await agent.post('/api/readings/weekday')
  ...
  it('returns 400 when weekday_aliyah_id is missing', async () => {
    const res = await agent.post('/api/readings/weekday').send({ date_read: '2024-01-01' });
    expect(res.status).toBe(400);
  });
...
describe('PUT /api/readings/weekday/:id', () => {
  it('updates date_read, note, and location', async () => {
    const aliyot = await agent.get('/api/weekday-aliyot');
    const post = await agent.post('/api/readings/weekday')
      .send({ weekday_aliyah_id: aliyot.body[0].id, date_read: '2024-01-01' });

    const res = await agent.put(`/api/readings/weekday/${post.body.id}`)
      .send({ date_read: '2024-02-01', note: 'n', location: 'Shul' });
    expect(res.status).toBe(200);

    const after = await agent.get('/api/weekday-aliyot');
  ...
  it('returns 400 when date_read is missing', async () => {
    ...
    const post = await agent.post('/api/readings/weekday')
      .send({ weekday_aliyah_id: aliyot.body[0].id, date_read: '2024-01-01' });
    const res = await agent.put(`/api/readings/weekday/${post.body.id}`).send({});
    expect(res.status).toBe(400);
  });

  it('returns 404 for a non-existent weekday reading', async () => {
    const res = await agent.put('/api/readings/weekday/999999').send({ date_read: '2024-01-01' });
    expect(res.status).toBe(404);
  });
});
...
describe('DELETE /api/readings/weekday/:id', () => {
  it('returns 204 and removes the weekday reading', async () => {
    ...
    const post = await agent.post('/api/readings/weekday')
      .send({ weekday_aliyah_id: aliyot.body[0].id, date_read: '2024-01-01' });
    const del = await agent.delete(`/api/readings/weekday/${post.body.id}`);
    expect(del.status).toBe(204);
  });

  it('returns 404 for a non-existent weekday reading', async () => {
    const res = await agent.delete('/api/readings/weekday/999999');
    expect(res.status).toBe(404);
  });
});
...
describe('GET /api/readings/hosafot', () => {
  it('returns an empty array when none recorded', async () => {
    const res = await agent.get('/api/readings/hosafot');
    expect(res.status).toBe(200);
    expect(res.body).toEqual([]);
  });

  it('reflects a posted hosafah reading', async () => {
    await agent.post('/api/readings/hosafot').send(HOSAFAH_BODY);
    const res = await agent.get('/api/readings/hosafot');
...
describe('POST /api/readings/hosafot', () => {
  it('returns 201 and an id on success', async () => {
    const res = await agent.post('/api/readings/hosafot').send(HOSAFAH_BODY);
    expect(res.status).toBe(201);
    expect(typeof res.body.id).toBe('number');
  });
  ...
  it('coerces is_double_parsha to 1/0', async () => {
    const post = await agent.post('/api/readings/hosafot')
  ...
  it('returns 400 when chapter_start is not numeric', async () => {
    const res = await agent.post('/api/readings/hosafot')
      .send({ ...HOSAFAH_BODY, chapter_start: 'abc' });
    expect(res.status).toBe(400);
    const after = await agent.get('/api/readings/hosafot');
...
describe('PUT /api/readings/hosafot/:id', () => {
  it('updates date_read, note, and location', async () => {
    const post = await agent.post('/api/readings/hosafot').send(HOSAFAH_BODY);
    const res = await agent.put(`/api/readings/hosafot/${post.body.id}`)
  ...
  it('returns 400 when date_read is missing', async () => {
    const post = await agent.post('/api/readings/hosafot').send(HOSAFAH_BODY);
    const res = await agent.put(`/api/readings/hosafot/${post.body.id}`).send({});
    expect(res.status).toBe(400);
  });

  it('returns 404 for a non-existent hosafah reading', async () => {
    const res = await agent.put('/api/readings/hosafot/999999').send({ date_read: '2024-01-01' });
    expect(res.status).toBe(404);
  });
});
...
describe('DELETE /api/readings/hosafot/:id', () => {
  it('returns 204 and removes the hosafah reading', async () => {
    const post = await agent.post('/api/readings/hosafot').send(HOSAFAH_BODY);
    const del = await agent.delete(`/api/readings/hosafot/${post.body.id}`);
    expect(del.status).toBe(204);
    const res = await agent.get('/api/readings/hosafot');
    expect(res.body).toEqual([]);
  });
  ...
  it('returns 404 for a non-existent hosafah reading', async () => {
    const res = await agent.delete('/api/readings/hosafot/999999');
    expect(res.status).toBe(404);
  });
});
...
describe('GET /api/calendar.ics', () => {
  it('returns an ICS feed with the expected headers and content', async () => {
    const res = await agent.get('/api/calendar.ics');
    expect(res.status).toBe(200);
